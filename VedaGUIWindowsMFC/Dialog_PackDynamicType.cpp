...
CDialog_PackDynamicType::CDialog_PackDynamicType(CWnd* pParent /*=NULL*/)
...
// Gestionnaires de messages CDialog_PackDynamicType
...
void	CDialog_PackDynamicType::Update(void)
{
	...
	SetCurrentShape( pDynamicType->Get() ); // can be null;
	...
	{	
		...
		//if( editorrec.Width()+12>thiswidth ) thiswidth = editorrec.Width()+12;
	...
	Resize(/*-1*/thiswidth+2,thisheight);
...
void	CDialog_PackDynamicType::SetCurrentShape(BaseType *_pCurrentShape)
{
	...
	if( m_pCurrentShapeDialog)
	{
		m_pCurrentShapeDialog->Create( /*&m_ListMemberEditorWindow*/ this);
		//m_pCurrentShapeDialog->ModifyStyle(0, WS_BORDER);
