// Dialog_PackListDynamic.cpp : fichier d'implémentation
//
...
CDialog_PackListDynamic::CDialog_PackListDynamic(CWnd* pParent /*=NULL*/)
...
void CDialog_PackListDynamic::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Control(pDX, IDC_LISTMEMBEREDITOR, m_ListMemberEditorWindow);
...
// Gestionnaires de messages CDialog_PackListDynamic
														   ...
														   )
{
		...
		)
	{
		...
		//m_BtClone.ShowWindow( SW_SHOW);
		//m_BtSelectAtPreviewTime.ShowWindow( SW_SHOW);
		//m_BtAddAtPreviewTime.ShowWindow( SW_SHOW);
	} else
	{	// classic list:
		//m_BtUp.ShowWindow( SW_HIDE);
		//m_BtDown.ShowWindow( SW_HIDE);
		//m_BtClone.ShowWindow( SW_HIDE);
		m_BtSelectAtPreviewTime.ShowWindow( SW_HIDE);
		m_BtAddAtPreviewTime.ShowWindow( SW_HIDE);
	}


	// note: we can only setup the list shape when a first element is created.
	Update();
}
...
BOOL CDialog_PackListDynamic::OnInitDialog()
{
	...
	//m_listbox.ModifyStyle(0, LVS_grid
	//m_listctrl.ModifyStyle(LVS_NOCOLUMNHEADER, LVS_REPORT | LVS_OWNERDRAWFIXED );
...
*/		
	...
	{	// if object is a PackStruct, "n" column:
		...
		while( pcell )
		{
			...
			if( pob->GetInfoString() ) // if editable member add column:
			{
				lvc.iSubItem = nbc;
	...
	{ // else it's a one object list:
			...
			lvc.mask = LVCF_TEXT | LVCF_SUBITEM | LVCF_WIDTH | LVCF_FMT ;
			lvc.cx = 160 ;	
			if( _pObjectToShapeList->GetInfoString() ) // if editable member add column:
			{
				lvc.iSubItem = 1;
...
void	CDialog_PackListDynamic::Update(void)
{
	...
	// get last selected
	int lastselected = m_listctrl.GetNextItem(-1, LVNI_SELECTED/*LVIS_FOCUSED*/ );
	// kepp current edited to check destruction:
	...
	while(plistcell)
	{
		...
		if( strcmp(peltobj->GetClassID(),"PackStruct") == 0 )
		{
			...
			while(psubcell)
			{
				...
				if( pob->GetInfoString() ) // if editable member write value:
				{
					lvi.iSubItem	= nbc;
		...
		} // end if the ob elt is a PackStruct.
		else
		{	// if list of a unique element:
			if( peltobj->GetInfoString() ) // if editable member write value:
			{
				lvi.iSubItem	= 1;
	...
	} // end of loop by list element.
	// if editeed element was destroyed,close:
	if( stillpresent == 0 )
	{
	...
	Resize(/*-1*/thiswidth+8,listrec.bottom +10 + editorrec.Height());
...
void CDialog_PackListDynamic::OnBnClickedButtonPlus()
{
	...
	if (pos != NULL) // return; //  TRACE0("No items were selected!\n");
	{
...
void CDialog_PackListDynamic::OnBnClickedButtonMinus()
{
	...
	// if destroyed is selected, unselect:

	if( m_pMemberEditor && m_pMemberEditor->GetManagedObject() == peltobj )
	{
		SetElementEditor( NULL );
	}
	// this should occur an update:
...
/*	BaseContext *pContext = peltobj->GetObjectThatManagesThis()->GetContext();
	pContext->NotifyObjectChange(BaseContext::eVedaUpdate_MemberChanged,
							peltobj->GetObjectThatManagesThis(),
							peltobj );

*/
	// bad bad update solution for resize:
...
void CDialog_PackListDynamic::OnBnClickedButtonUp()
{
	...
	int ii=0; // find index... ugly method...
	while(pCell)
	{
...
void CDialog_PackListDynamic::OnBnClickedButtonAddatptime()
{
	...
	if(classID.Compare("PackList::PackList_TimeTrack")==0)
	{
		...
		for(ii=0;ii<m_listctrl.GetItemCount() ; ii++)
		{
			...
			if(peltobj==pCell->GetManagedObject())
			{
				PackList::Cell *pnCell = pVedaList->AddElement(ii+1); // watch the +1 !!
				if(pnCell)
				{
					((PackList_TimeTrack::TimeTrackElement *) pnCell->GetManagedObject())
						->SetTimeInSecond(prevtime);
					SetElementEditor( pnCell->GetManagedObject() );// set editor to new element:
					Update();
				}
				return;
			}
