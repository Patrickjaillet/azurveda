...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
    stream->main_data = (unsigned char *)malloc(MAD_BUFFER_MDLEN);
    if (stream->main_data == 0) {
  ...
  if (si.main_data_begin == 0) {
	...
	result = -1;
      ...
      mad_bit_init(&ptr, (const unsigned char *)
	...
	memcpy( (void *)(stream->main_data + stream->md_len),
...
# endif
  ...
  if (frame_free >= next_md_begin) {
	...
	memmove((void*)(stream->main_data),
		(const void*)(stream->main_data + stream->md_len - extra), extra);
	stream->md_len = extra;
      }
    }
    else
